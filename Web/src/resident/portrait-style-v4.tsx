...
export type PortraitStyleId = 'paper-cut' | 'ink-flat' | 'woodblock' | 'painterly' | 'chibi' | 'baimiao' | 'clay' | 'folk-print';
...
export const PORTRAIT_STYLES: Array<{
  ...
  { id: 'chibi', name: '克制 Q 版', shortName: 'Q版', note: '放大头部与关键轮廓，不做幼儿化大眼萌脸，观察高频 UI 中的亲和力。' },
  { id: 'baimiao', name: '白描人物', shortName: '白描', note: '纸色底 + 细线描，接近人物谱与传统白描，测试最轻量的古风表达。' },
  { id: 'clay', name: '陶俑泥塑', shortName: '陶俑', note: '陶土单色与浅浮雕体块，强调古朴、实体感和博物志气质。' },
  { id: 'folk-print', name: '民俗年画套色', shortName: '年画套色', note: '更鲜明的传统套色与粗线结构，测试民俗感与城市烟火气。' },
];
...
function hairBack(asset: HairAssetId, color: string, style: PortraitStyleId) {
  const stroke = style === 'woodblock' ? '#2b231d' : style === 'folk-print' ? '#4a2d24' : style === 'baimiao' ? '#57493c' : style === 'ink-flat' ? '#43362e' : 'none';
  const sw = style === 'woodblock' ? 2.2 : style === 'folk-print' ? 1.8 : style === 'baimiao' ? 1.05 : style === 'ink-flat' ? 1.2 : 0;
...
function hairFront(asset: HairAssetId, color: string, style: PortraitStyleId) {
  const outline = style === 'woodblock' ? '#2b231d' : style === 'folk-print' ? '#4a2d24' : style === 'baimiao' ? '#57493c' : style === 'ink-flat' ? '#44362d' : 'none';
  const sw = style === 'woodblock' ? 2.1 : style === 'folk-print' ? 1.75 : style === 'baimiao' ? 1.05 : style === 'ink-flat' ? 1.15 : 0;
...
function outfit(spec: GoldenPortraitSpec, style: PortraitStyleId, cloth: string, accent: string) {
  const outline = style === 'woodblock' ? '#2e251f' : style === 'folk-print' ? '#4a2d24' : style === 'baimiao' ? '#655546' : style === 'ink-flat' ? '#4a3c32' : 'none';
  const sw = style === 'woodblock' ? 2.4 : style === 'folk-print' ? 2 : style === 'baimiao' ? 1.1 : style === 'ink-flat' ? 1.25 : 0;
  const female = spec.gender === 'female';
  const chibi = style === 'chibi';
  const left = chibi ? (female ? 27 : 24) : (female ? 19 : 15);
  const right = chibi ? (female ? 93 : 96) : (female ? 101 : 105);
  const top = chibi ? 96 : (female ? 91 : 89);
...
function beard(spec: GoldenPortraitSpec, color: string, style: PortraitStyleId) {
  ...
  const stroke = style === 'woodblock' ? '#2c231e' : style === 'folk-print' ? '#43261f' : color;
...
function accessory(spec: GoldenPortraitSpec, style: PortraitStyleId) {
  ...
  const metal = style === 'woodblock' ? '#6e4f35' : style === 'folk-print' ? '#c99b3e' : style === 'clay' ? '#b17855' : '#b8a56f';
...
function ageMarks(spec: GoldenPortraitSpec, ink: string, style: PortraitStyleId) {
  ...
  const opacity = style === 'woodblock' ? .55 : style === 'folk-print' ? .48 : style === 'clay' ? .24 : style === 'painterly' ? .28 : .38;
  return <g fill="none" stroke={ink} strokeWidth={style === 'woodblock' ? 1.2 : style === 'folk-print' ? 1.05 : .75} opacity={opacity}>
...
function faceDetails(asset: FaceAsset, style: PortraitStyleId, ink: string) {
  if (style === 'chibi') return <g fill="none" stroke={ink} strokeLinecap="round">
    <path d={asset.leftBrow} strokeWidth="1.8" opacity=".58"/><path d={asset.rightBrow} strokeWidth="1.8" opacity=".58"/>
    <path d={asset.leftEye} strokeWidth="1.6"/><path d={asset.rightEye} strokeWidth="1.6"/>
    <circle cx="50" cy="53" r="1.25" fill={ink} stroke="none" opacity=".78"/><circle cx="70" cy="53" r="1.25" fill={ink} stroke="none" opacity=".78"/>
    <path d={asset.nose} strokeWidth=".8" opacity=".34"/><path d={asset.mouth} strokeWidth="1.35" opacity=".68"/>
  </g>;
  if (style === 'baimiao') return <g fill="none" stroke={ink} strokeLinecap="round">
    <path d={asset.leftBrow} strokeWidth="1.15" opacity=".8"/><path d={asset.rightBrow} strokeWidth="1.15" opacity=".8"/>
    <path d={asset.leftEye} strokeWidth=".92"/><path d={asset.rightEye} strokeWidth=".92"/>
    <path d={asset.nose} strokeWidth=".78" opacity=".72"/><path d={asset.mouth} strokeWidth=".86" opacity=".8"/>
    {asset.cheekLeft && <path d={asset.cheekLeft} strokeWidth=".55" opacity=".32"/>}
    {asset.cheekRight && <path d={asset.cheekRight} strokeWidth=".55" opacity=".32"/>}
  </g>;
  if (style === 'clay') return <g fill="none" stroke={ink} strokeLinecap="round" opacity=".5">
    <path d={asset.leftBrow} strokeWidth="1.4"/><path d={asset.rightBrow} strokeWidth="1.4"/>
    <path d={asset.leftEye} strokeWidth="1.1"/><path d={asset.rightEye} strokeWidth="1.1"/>
    <path d={asset.nose} strokeWidth="1.05"/><path d={asset.mouth} strokeWidth="1.1"/>
  </g>;
  if (style === 'folk-print') return <g fill="none" stroke={ink} strokeLinecap="round">
    <path d={asset.leftBrow} strokeWidth="2.3"/><path d={asset.rightBrow} strokeWidth="2.3"/>
    <path d={asset.leftEye} strokeWidth="1.65"/><path d={asset.rightEye} strokeWidth="1.65"/>
    <path d={asset.nose} strokeWidth="1.25" opacity=".8"/><path d={asset.mouth} strokeWidth="1.55"/>
    <circle cx="46" cy="64" r="2.4" fill="#a94e3f" stroke="none" opacity=".24"/><circle cx="74" cy="64" r="2.4" fill="#a94e3f" stroke="none" opacity=".24"/>
  </g>;
...
function styleColors(spec: GoldenPortraitSpec, style: PortraitStyleId) {
  ...
  if (style === 'chibi') return { background: '#4a5750', skin: spec.skin, hair: spec.hair, cloth: spec.cloth, ink: '#42342f', accent: '#d5c39c' };
  if (style === 'baimiao') return { background: '#eee5d2', skin: '#ead8be', hair: '#51453b', cloth: '#ddd0b7', ink: '#5a4a3d', accent: '#8f7962' };
  if (style === 'clay') return { background: '#6b5849', skin: '#ad7254', hair: '#68483b', cloth: '#8c5f49', ink: '#5b392c', accent: '#c18b68' };
  if (style === 'folk-print') return { background: '#d9c083', skin: '#d49a72', hair: '#302823', cloth: spec.wealth === 'wealthy' ? '#8d3f38' : spec.cloth, ink: '#442b23', accent: '#dfbb58' };
...
export function PortraitStylePortrait({ spec, artStyle, label }: { spec: GoldenPortraitSpec; artStyle: PortraitStyleId; label?: string; }) {
  ...
  const faceOutline = artStyle === 'woodblock' ? colors.ink : artStyle === 'folk-print' ? colors.ink : artStyle === 'baimiao' ? colors.ink : artStyle === 'ink-flat' ? colors.ink : artStyle === 'clay' ? '#754b39' : 'none';
  const faceStrokeWidth = artStyle === 'woodblock' ? 2.4 : artStyle === 'folk-print' ? 1.8 : artStyle === 'baimiao' ? 1 : artStyle === 'ink-flat' ? 1.05 : artStyle === 'clay' ? .7 : 0;
  const chibi = artStyle === 'chibi';
  const neckTop = chibi ? 82 : (spec.gender === 'female' ? 81 : 83);
  ...
  >
    <svg viewBox="0 15 120 120" focusable="false">
      <defs>
        ...
        <pattern id={uid + '-clay-speckle'} width="10" height="10" patternUnits="userSpaceOnUse">
          <circle cx="2" cy="3" r=".7" fill="#4e3026" opacity=".16"/><circle cx="8" cy="7" r=".55" fill="#d49a76" opacity=".18"/>
        </pattern>
      </defs>
      ...
      <g transform={chibi ? 'translate(60 64) scale(1.18) translate(-60 -58)' : undefined}>
        {hairBack(spec.hairAsset, colors.hair, artStyle)}
        <path d={'M' + (60 - asset.neckHalf) + ' ' + neckTop + ' H' + (60 + asset.neckHalf) + ' L' + (60 + asset.neckHalf + 1.5) + ' ' + neckBottom + ' Q60 104 ' + (60 - asset.neckHalf - 1.5) + ' ' + neckBottom + 'Z'} fill={(artStyle === 'painterly' || artStyle === 'clay') ? 'url(#' + uid + '-skin)' : colors.skin} stroke={faceOutline} strokeWidth={faceStrokeWidth}/>
        ...
        <path d={asset.path} fill={(artStyle === 'painterly' || artStyle === 'clay') ? 'url(#' + uid + '-skin)' : colors.skin} stroke={faceOutline} strokeWidth={faceStrokeWidth} strokeLinejoin="round"/>
        ...
        {artStyle === 'baimiao' && <><path d="M45 39 Q60 30 75 39" fill="none" stroke={colors.ink} strokeWidth=".55" opacity=".34"/><path d="M47 72 Q60 82 73 72" fill="none" stroke={colors.ink} strokeWidth=".45" opacity=".22"/></>}
        {artStyle === 'clay' && <><path d={asset.path} fill={'url(#' + uid + '-clay-speckle)'} opacity=".55"/><path d="M44 42 Q49 32 60 30" fill="none" stroke="#d39a76" strokeWidth="1.1" opacity=".18"/></>}
        {artStyle === 'folk-print' && <><path d="M44 39 Q60 29 76 39" fill="none" stroke="#f0cc69" strokeWidth="1.25" opacity=".5"/><path d="M46 78 Q60 84 74 78" fill="none" stroke="#a84b3d" strokeWidth=".85" opacity=".48"/></>}
      ...
      {artStyle === 'baimiao' && <g fill="none" stroke={colors.ink} strokeWidth=".55" opacity=".3"><path d="M29 122 Q45 111 54 113"/><path d="M66 113 Q79 114 92 126"/></g>}
      {artStyle === 'clay' && <g fill={'url(#' + uid + '-clay-speckle)'} opacity=".32"><path d="M22 111 Q60 94 98 111 L104 150 H16Z"/></g>}
      {artStyle === 'folk-print' && <g fill="none" stroke="#b24d3f" opacity=".36"><path d="M21 128 Q60 116 99 128" strokeWidth="1.1"/><path d="M27 135 Q60 124 93 135" strokeWidth=".75"/></g>}
    </svg>
  </div>;
}
