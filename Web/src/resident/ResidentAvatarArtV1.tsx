...
const FALLBACK_MALE_FACES = ['appearance.face.male-oval-01', 'appearance.face.male-round-01', 'appearance.face.male-long-01', 'appearance.face.male-square-01'];
const FALLBACK_FEMALE_FACES = ['appearance.face.female-youth-01', 'appearance.face.female-oval-01', 'appearance.face.female-adult-01', 'appearance.face.female-mature-01'];
const FALLBACK_MALE_HAIR = ['appearance.hair.male-short-01', 'appearance.hair.male-short-02', 'appearance.hair.male-tied-01', 'appearance.hair.male-tied-02'];
const FALLBACK_FEMALE_HAIR = ['appearance.hair.female-long-straight-01', 'appearance.hair.female-long-tied-01', 'appearance.hair.female-braid-01', 'appearance.hair.female-low-bun-01'];
...
const FALLBACK_HAIR_COLORS = ['appearance.palette.hair.black-01', 'appearance.palette.hair.black-02', 'appearance.palette.hair.dark-brown-01', 'appearance.palette.hair.brown-black-01'];
...
function fallbackAppearance(seed: number, gender: Gender, lifeStage: LifeStageId, occupationId: string): ResidentAppearanceDNA {
  ...
  return {
    ...
    hairPaletteId: lifeStage === 'elder' ? (seed % 2 ? 'appearance.palette.hair.salt-pepper-01' : 'appearance.palette.hair.silver-01') : pick(FALLBACK_HAIR_COLORS, seed, 29),
...
function HairBack({ id, color, rig }: { id: string; color: string; rig: PortraitRig }) {
  ...
  if (id.includes('female-long-straight-01')) return <><path d={`M${left + 2} ${rig.hairlineY + 7} Q${left - 6} 76 ${left + 1} 139 L${left + 15} 142 Q${left + 9} 92 ${left + 12} ${rig.hairlineY + 18}Z`} fill={color}/><path d={`M${right - 2} ${rig.hairlineY + 7} Q${right + 6} 76 ${right - 1} 139 L${right - 15} 142 Q${right - 9} 92 ${right - 12} ${rig.hairlineY + 18}Z`} fill={color}/>{cap}</>;
  if (id.includes('female-long-straight-02')) return <><path d={`M${left + 1} ${rig.hairlineY + 9} C${left - 8} 70 ${left - 4} 111 ${left + 2} 145 L${left + 17} 143 C${left + 9} 105 ${left + 11} 70 ${left + 13} ${rig.hairlineY + 18}Z`} fill={color}/><path d={`M${right - 1} ${rig.hairlineY + 9} C${right + 8} 70 ${right + 4} 111 ${right - 2} 145 L${right - 17} 143 C${right - 9} 105 ${right - 11} 70 ${right - 13} ${rig.hairlineY + 18}Z`} fill={color}/>{cap}</>;
  if (id.includes('female-long-tied')) return <><path d={`M${rig.centerX + 7} ${rig.topY + 4} Q${right + 11} 76 ${right + 2} 139 Q${rig.centerX + 13} 127 ${rig.centerX + 10} 55Z`} fill={color}/><ellipse cx={rig.centerX + 13} cy={rig.topY + 1} rx="7" ry="5" fill={color}/>{cap}</>;
  if (id.includes('female-side-fall')) return <><path d={`M${right - 6} ${rig.hairlineY + 8} Q${right + 8} 78 ${right + 1} 143 L${right - 11} 145 Q${right - 4} 91 ${right - 14} ${rig.hairlineY + 18}Z`} fill={color}/>{cap}</>;
  if (id.includes('female-double-braid')) return <><path d={`M${left + 6} ${rig.earY + 3} Q${left - 6} 91 ${left + 1} 140`} fill="none" stroke={color} strokeWidth="7" strokeLinecap="round"/><path d={`M${right - 6} ${rig.earY + 3} Q${right + 6} 91 ${right - 1} 140`} fill="none" stroke={color} strokeWidth="7" strokeLinecap="round"/>{cap}</>;
  if (id.includes('female-braid')) return <><path d={`M${right - 5} ${rig.earY + 3} Q${right + 6} 91 ${right - 1} 141`} fill="none" stroke={color} strokeWidth="8" strokeLinecap="round"/><circle cx={right - 1} cy="141" r="4" fill={color}/>{cap}</>;
  if (id.includes('female-married-bun')) return <><ellipse cx={rig.centerX + 6} cy={rig.topY - 3} rx="13" ry="8" fill={color}/><ellipse cx={rig.centerX - 6} cy={rig.topY + 1} rx="9" ry="7" fill={color}/>{cap}</>;
  ...
  if (id.includes('male-thinning')) return <><ellipse cx={rig.centerX + 7} cy={rig.topY + 5} rx="11" ry="5" fill={color} opacity=".8"/></>;
...
function HairFront({ id, color, rig }: { id: string; color: string; rig: PortraitRig }) {
  ...
  if (id.includes('male-thinning')) return <path d={`M${left + 4} ${line + 2} Q${left + 9} ${top + 4} ${rig.centerX - 5} ${top + 3} M${right - 4} ${line + 1} Q${right - 8} ${top + 5} ${rig.centerX + 8} ${top + 3}`} fill="none" stroke={color} strokeWidth="4.5" strokeLinecap="round"/>;
  if (id.includes('female-married-bun')) return <g fill={color}><path d={`M${left} ${line + 1} Q${left + 2} ${top} ${rig.centerX} ${top} Q${right - 2} ${top} ${right} ${line + 1} Q${rig.centerX + 8} ${line - 4} ${rig.centerX} ${line - 7} Q${rig.centerX - 8} ${line - 4} ${left} ${line + 1}Z`}/><path d={`M${left + 3} ${line} Q${left + 1} ${rig.eyeY + 5} ${left + 7} ${rig.noseY + 4} Q${left + 11} ${rig.eyeY + 3} ${left + 11} ${line + 2}Z`}/></g>;
...
function Outfit({ id, color, rig }: { id: string; color: string; rig: PortraitRig }) {
  ...
  const wealthy = id.includes('wealthy') || id.includes('refined');
  const comfortable = id.includes('comfortable') || id.includes('layered');
  const rough = id.includes('rough');
  const home = id.includes('home');
  const merchant = id.includes('merchant');
  const shoulderHalf = female ? (merchant ? 39 : 37) : rough ? 40 : wealthy ? 37 : 36;
  const topY = rig.shoulderY - 4;
  const collar = wealthy ? '#d3c7a8' : comfortable ? '#beb39b' : '#afa48f';
  const inner = wealthy ? '#8f826d' : '#8c8170';

  return <g>
    <path d={`M${rig.centerX - shoulderHalf} 150 Q${rig.centerX - shoulderHalf + (female ? 1 : 3)} ${topY + (female ? 7 : 3)} ${rig.centerX - 12} ${topY} H${rig.centerX + 12} Q${rig.centerX + shoulderHalf - (female ? 1 : 3)} ${topY + (female ? 7 : 3)} ${rig.centerX + shoulderHalf} 150Z`} fill={color}/>
    {rough && <path d={`M${rig.centerX - shoulderHalf + 3} 146 L${rig.centerX - shoulderHalf + 9} ${topY + 14}`} stroke="#d5c7aa" strokeWidth="1" opacity=".12"/>}
    {home && <path d={`M${rig.centerX - 26} ${topY + 25} Q${rig.centerX} ${topY + 34} ${rig.centerX + 26} ${topY + 25}`} fill="none" stroke="#d8cdb4" strokeWidth="1" opacity=".16"/>}
    <path d={`M${rig.centerX - 11} ${topY} L${rig.centerX} ${topY + 14} L${rig.centerX + 11} ${topY}`} fill="none" stroke={collar} strokeWidth={wealthy ? 6.5 : rough ? 4.2 : 5.2} strokeLinejoin="round" opacity=".9"/>
    {comfortable && <path d={`M${rig.centerX - 18} ${topY + 10} Q${rig.centerX} ${topY + 23} ${rig.centerX + 18} ${topY + 10}`} fill="none" stroke={inner} strokeWidth={wealthy ? 3 : 2} opacity=".72"/>}
    {wealthy && <><path d={`M${rig.centerX - 23} ${topY + 24} Q${rig.centerX} ${topY + 33} ${rig.centerX + 23} ${topY + 24}`} fill="none" stroke="#d9caa7" strokeWidth="1.5" opacity=".38"/><path d={`M${rig.centerX - 29} ${topY + 33} H${rig.centerX + 29}`} stroke="#c8b587" strokeWidth="1.2" opacity=".24"/></>}
    ...
    <path d={`M${rig.centerX} ${topY + 14} V148`} stroke="#262924" strokeWidth="1.2" opacity=".16"/>
  </g>;
}

function AgeOverlay({ lifeStage, rig }: { lifeStage: LifeStageId; rig: PortraitRig }) {
  if (lifeStage !== 'middle-age' && lifeStage !== 'elder') return null;
  const elder = lifeStage === 'elder';
  const eyeOffset = rig.faceWidth * .22;
  return <g fill="none" stroke="#705745" strokeWidth={elder ? .82 : .72} opacity={elder ? '.38' : '.22'}>
    ...
    {elder && <><path d={`M${rig.centerX - rig.jawWidth * .28} ${rig.mouthY + 4} Q${rig.centerX} ${rig.mouthY + 8} ${rig.centerX + rig.jawWidth * .28} ${rig.mouthY + 4}`}/><path d={`M${rig.centerX - rig.faceWidth * .36} ${rig.noseY + 2} Q${rig.centerX - rig.faceWidth * .3} ${rig.mouthY} ${rig.centerX - rig.faceWidth * .24} ${rig.mouthY + 2}`}/><path d={`M${rig.centerX + rig.faceWidth * .36} ${rig.noseY + 2} Q${rig.centerX + rig.faceWidth * .3} ${rig.mouthY} ${rig.centerX + rig.faceWidth * .24} ${rig.mouthY + 2}`}/></>}
  </g>;
}
...
export function ResidentAvatar({ seed = 1, gender, lifeStage, occupationId = '', appearance, catalog, label, debugRig = false, hiddenLayers = [], masterPreview = false }: Props) {
  ...
  return <div className={`generated-portrait generated-portrait--svg ${masterPreview ? 'generated-portrait--master' : 'generated-portrait--crop'} gender-${gender} age-${lifeStage}`} data-appearance-signature={appearanceSignature(dna)} data-face-family={faceFamily} data-hair-visibility={hairVisibility} data-hair-silhouette={hairSilhouette} data-outfit-silhouette={outfitSilhouette} data-master-ratio="4:5" data-safe-area="1:1" data-rig-state={diagnostics.errors.length ? 'error' : 'ok'} data-rig-errors={diagnostics.errors.length} data-rig-warnings={diagnostics.warnings.length} data-debug-rig={debugRig ? 'true' : 'false'} role={label ? 'img' : undefined} aria-label={label} aria-hidden={label ? undefined : true}>
    <svg viewBox={`0 0 ${PORTRAIT_MASTER.width} ${PORTRAIT_MASTER.height}`} focusable="false" preserveAspectRatio="xMidYMid slice">
      <defs><linearGradient id={`portrait-bg-${Math.abs(seed)}-${masterPreview ? 'm' : 'c'}`} x1="0" y1="0" x2="1" y2="1"><stop offset="0" stopColor="#3b4740"/><stop offset="1" stopColor="#293630"/></linearGradient></defs>
      <rect width={PORTRAIT_MASTER.width} height={PORTRAIT_MASTER.height} rx="22" fill={`url(#portrait-bg-${Math.abs(seed)}-${masterPreview ? 'm' : 'c'})`}/>
      <circle cx="60" cy="57" r="49" fill="#d7cba8" opacity=".055"/>
      {showHairBack && <HairBack id={dna.hairId} color={hair} rig={rig}/>} 
      {!hidden.has('outfit') && <Outfit id={dna.outfitId} color={cloth} rig={rig}/>} 
      <path d={`M${rig.centerX - neckHalf} ${rig.neckTopY} H${rig.centerX + neckHalf} L${rig.centerX + neckHalf + 1} ${rig.shoulderY + 8} Q${rig.centerX} ${rig.shoulderY + 12} ${rig.centerX - neckHalf - 1} ${rig.shoulderY + 8}Z`} fill={skin}/>
      <ellipse cx={rig.centerX - half - 1} cy={rig.earY} rx="4.3" ry="6.2" fill={skin}/><ellipse cx={rig.centerX + half + 1} cy={rig.earY} rx="4.3" ry="6.2" fill={skin}/><path d={facePath(rig, faceFamily)} fill={skin}/>
      ...
      <path d={`M${rig.centerX} ${rig.eyeY} Q${rig.centerX - 3} ${rig.noseY} ${rig.centerX} ${rig.noseY + 1} Q${rig.centerX + 3} ${rig.noseY + 1} ${rig.centerX + 4} ${rig.noseY - 1}`} fill="none" stroke="#855f4a" strokeWidth="1.1" strokeLinecap="round" opacity=".48"/><path d={`M${rig.centerX - rig.jawWidth * .22} ${rig.mouthY} Q${rig.centerX} ${rig.mouthY + 3} ${rig.centerX + rig.jawWidth * .22} ${rig.mouthY}`} fill="none" stroke="#7d4f45" strokeWidth="1.5" strokeLinecap="round" opacity=".72"/>
      ...
      {!hidden.has('age-overlay') && <AgeOverlay lifeStage={lifeStage} rig={rig}/>} 
