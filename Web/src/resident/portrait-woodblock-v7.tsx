...
function facePath(rig: PortraitRig, spec: WoodblockPortraitSpec) {
  ...
  const left = cx - half;
  const right = cx + half;
  const childRound = spec.lifeStage === 'child' ? 2 : 0;
  const workerSquare = spec.faceProfileId === 'female-worker-broad' ? 2 : 0;

  if (spec.gender === 'female') {
    const chinHalf = spec.lifeStage === 'child' ? 6 : spec.lifeStage === 'elder' ? 4.5 : 5;
    return [
      'M', cx, rig.topY,
      'C', left + 6, rig.topY - 1, left, rig.topY + 12, left + 1, rig.eyeY + 5,
      'C', left + 1, rig.mouthY + 4, cx - jawHalf - workerSquare, rig.chinY - 9, cx - chinHalf, rig.chinY,
      'Q', cx, rig.chinY + childRound + 1, cx + chinHalf, rig.chinY,
      'C', cx + jawHalf + workerSquare, rig.chinY - 9, right - 1, rig.mouthY + 4, right - 1, rig.eyeY + 5,
  ...
  return [
    'M', cx, rig.topY,
    'C', left + 6, rig.topY - 1, left, rig.topY + 12, left + 1, rig.eyeY + 5,
    'C', left + 1, rig.mouthY + 5, cx - jawHalf, rig.chinY - 10, cx, rig.chinY + childRound,
    'C', cx + jawHalf, rig.chinY - 10, right - 1, rig.mouthY + 5, right - 1, rig.eyeY + 5,
    'C', right, rig.topY + 12, right - 6, rig.topY - 1, cx, rig.topY,
    'Z',
  ].join(' ');
}
...
function HairFront({ spec, rig, size }: { spec: WoodblockPortraitSpec; rig: PortraitRig; size?: PortraitSize }) {
  ...
  if (spec.hairId === 'girl-double-bun' || spec.hairId === 'girl-ribbon-side') {
    const top = a.skullTop.y + 1;
    const templeY = a.templeLeft.y;
    return <path d={'M' + left + ' ' + (templeY + 2) +
      ' Q' + (left + 4) + ' ' + top + ' ' + rig.centerX + ' ' + (top - 1) +
      ' Q' + (right - 4) + ' ' + top + ' ' + right + ' ' + (templeY + 2) +
      ' Q' + (rig.centerX + 9) + ' ' + (templeY - 3) + ' ' + rig.centerX + ' ' + (templeY - 4) +
      ' Q' + (rig.centerX - 9) + ' ' + (templeY - 3) + ' ' + left + ' ' + (templeY + 2) + 'Z'} fill={spec.hair}/>;
  }
...
function FaceDetails({ spec, rig }: { spec: WoodblockPortraitSpec; rig: PortraitRig }) {
  ...
  if (female) {
    const browHalf = child ? 4.6 : 5.2;
    const eyeHalf = child ? 3.5 : 4.2;
    const browLift = child ? 1.8 : 2.2;
    const noseEnd = child ? 3 : spec.lifeStage === 'elder' ? 4 : 3.6;
    const mouthHalf = child ? 5.5 : spec.lifeStage === 'middle' || spec.lifeStage === 'elder' ? 6.3 : 6;
    return <g fill="none" stroke={LINE} strokeLinecap="round">
      <path d={'M' + (cx - eyeSpan - browHalf) + ' ' + rig.browY + ' Q' + (cx - eyeSpan) + ' ' + (rig.browY - browLift) + ' ' + (cx - eyeSpan + browHalf) + ' ' + rig.browY} strokeWidth={child ? 1.75 : 2.05}/>
      <path d={'M' + (cx + eyeSpan - browHalf) + ' ' + rig.browY + ' Q' + (cx + eyeSpan) + ' ' + (rig.browY - browLift) + ' ' + (cx + eyeSpan + browHalf) + ' ' + rig.browY} strokeWidth={child ? 1.75 : 2.05}/>
      <path d={'M' + (cx - eyeSpan - eyeHalf) + ' ' + eyeY + ' Q' + (cx - eyeSpan) + ' ' + (eyeY - 2.4) + ' ' + (cx - eyeSpan + eyeHalf) + ' ' + eyeY} strokeWidth={child ? 1.45 : 1.7}/>
      <path d={'M' + (cx + eyeSpan - eyeHalf) + ' ' + eyeY + ' Q' + (cx + eyeSpan) + ' ' + (eyeY - 2.4) + ' ' + (cx + eyeSpan + eyeHalf) + ' ' + eyeY} strokeWidth={child ? 1.45 : 1.7}/>
      {!child && <><circle cx={cx - eyeSpan} cy={eyeY - .2} r=".65" fill={LINE}/><circle cx={cx + eyeSpan} cy={eyeY - .2} r=".65" fill={LINE}/></>}
      <path d={'M' + (cx - .3) + ' ' + (eyeY + 2) + ' Q' + (cx - 1.5) + ' ' + (rig.noseY - 2) + ' ' + cx + ' ' + rig.noseY + ' Q' + (cx + 1.8) + ' ' + (rig.noseY + 1.5) + ' ' + (cx + noseEnd) + ' ' + (rig.noseY - .5)} strokeWidth={child ? 1.0 : 1.25}/>
      <path d={'M' + (cx - mouthHalf) + ' ' + rig.mouthY + ' Q' + cx + ' ' + (rig.mouthY + (child ? 2 : 2.8)) + ' ' + (cx + mouthHalf) + ' ' + rig.mouthY} strokeWidth={child ? 1.25 : 1.55}/>
    </g>;
  }

  const browWidth = child ? 9 : 11;
  const eyeWidth = child ? 7 : 9;
  return <g fill="none" stroke={LINE} strokeLinecap="square">
    <path d={'M' + (cx - eyeSpan - browWidth / 2) + ' ' + rig.browY + ' Q' + (cx - eyeSpan) + ' ' + (rig.browY - 3.2) + ' ' + (cx - eyeSpan + browWidth / 2) + ' ' + rig.browY} strokeWidth={child ? 2.1 : 3.1}/>
    <path d={'M' + (cx + eyeSpan - browWidth / 2) + ' ' + rig.browY + ' Q' + (cx + eyeSpan) + ' ' + (rig.browY - 3.2) + ' ' + (cx + eyeSpan + browWidth / 2) + ' ' + rig.browY} strokeWidth={child ? 2.1 : 3.1}/>
    <path d={'M' + (cx - eyeSpan - eyeWidth / 2) + ' ' + eyeY + ' Q' + (cx - eyeSpan) + ' ' + (eyeY - 3) + ' ' + (cx - eyeSpan + eyeWidth / 2) + ' ' + eyeY} strokeWidth={child ? 1.7 : 2.1}/>
    <path d={'M' + (cx + eyeSpan - eyeWidth / 2) + ' ' + eyeY + ' Q' + (cx + eyeSpan) + ' ' + (eyeY - 3) + ' ' + (cx + eyeSpan + eyeWidth / 2) + ' ' + eyeY} strokeWidth={child ? 1.7 : 2.1}/>
    {!child && <><circle cx={cx - eyeSpan} cy={eyeY - .2} r=".8" fill={LINE}/><circle cx={cx + eyeSpan} cy={eyeY - .2} r=".8" fill={LINE}/></>}
    <path d={'M' + (cx - .5) + ' ' + (eyeY + 1) + ' Q' + (cx - 3) + ' ' + (rig.noseY - 2) + ' ' + cx + ' ' + rig.noseY + ' Q' + (cx + 4) + ' ' + (rig.noseY + 2) + ' ' + (cx + 6) + ' ' + (rig.noseY - 1)} strokeWidth={child ? 1.2 : 1.65}/>
    <path d={'M' + (cx - (child ? 6 : 8)) + ' ' + rig.mouthY + ' Q' + cx + ' ' + (rig.mouthY + (child ? 2.2 : 4)) + ' ' + (cx + (child ? 6 : 8)) + ' ' + rig.mouthY} strokeWidth={child ? 1.4 : 1.9}/>
  </g>;
}
...
function Accessory({ spec, rig }: { spec: WoodblockPortraitSpec; rig: PortraitRig }) {
  ...
  let target = a.bunMid;
  if (spec.hairId === 'adult-low-bun' || spec.hairId === 'elder-gray-low-bun' || spec.hairId === 'worker-tight-bun' || spec.hairId === 'young-low-tie') target = a.bunLow;
  if (spec.hairId === 'young-halfbound-backfall' || spec.hairId === 'young-halfbound-asym') target = a.crownBack;
  ...
  if (spec.accessory === 'double-prong-pin') {
    return <g stroke={pinColor} strokeLinecap="round">
      <path d={'M' + (target.x - 8) + ' ' + (target.y - 4) + ' L' + (target.x + 7) + ' ' + (target.y + 1)} strokeWidth="1.45"/>
      <path d={'M' + (target.x - 7) + ' ' + target.y + ' L' + (target.x + 7) + ' ' + (target.y + 4)} strokeWidth="1.15"/>
    </g>;
  }

  return <g>
    <path d={'M' + (target.x - 7) + ' ' + (target.y - 3) + ' L' + (target.x + 8) + ' ' + (target.y + 2)} stroke={pinColor} strokeWidth={spec.accessory === 'wood-pin' ? 1.65 : 1.45} strokeLinecap="round"/>
    {spec.accessory === 'jade-pin' && <circle cx={target.x + 9} cy={target.y + 2.2} r="1.9" fill="#788f72" stroke={LINE} strokeWidth=".55"/>}
    {spec.accessory === 'simple-hairpin' && <circle cx={target.x + 8.5} cy={target.y + 2} r="1.35" fill={pinColor}/>}
  </g>;
}
