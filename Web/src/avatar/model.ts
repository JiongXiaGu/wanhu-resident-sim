// 头像工坊只保存四个外观选择。对象的性别/年龄来自调用方，不是捏脸滑杆。
...
export type Recipe = { schema: 'wanhu.avatar'; version: 1; pack: 'linework-v1' } & Choices;
...
export function parseRecipe(value: unknown): Recipe {
  ...
  if (v.schema !== 'wanhu.avatar' || v.version !== 1 || v.pack !== 'linework-v1') throw new Error('此配方不属于当前头像工坊。');
  ...
  return { schema:'wanhu.avatar',version:1,pack:'linework-v1',...Object.fromEntries(parts.map(part => [part,v[part]])) } as Recipe;
}
...
export function randomRecipe(seed: number, previous?: Recipe): Recipe {
  ...
  const value: Record<string, unknown> = { ...defaultRecipe };
