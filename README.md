...
也可在首页点左下角 **头像工坊**，或在任意居民的身份栏点 **编辑头像**。

这一版是真实部件组合与对象保存：只有 **脸型、头发、衣服、表情** 四类。不是切换整张角色图，也不再提供帽子、调色或风格画廊。第一套原创 SVG 包“日常线绘”有 4 脸、6 发、4 衣、6 表情，支持男女儿童/成人/老人上下文。

编辑器左边选择玩家示例档案或当前城市任意居民，右边选择四类部件。点击 **应用** 才写入这个对象；未保存修改可撤销，切对象或关闭会提示。给甲改头像不会改变乙。关闭工坊后，实际居民信息面板显示刚应用的头像，游戏日期和故事不会重置。
...
当前 Visual Review 保留居民故事与正式 72 组合检查，新增工坊的玩家/居民保存、草稿、恢复、导入、错误处理和年龄/部件检查。`resident-visual-review` Artifact 的 `avatar/` 包含：

- 实际编辑窗口与已应用居民面板截图。
- 六上下文的脸型、表情、发型、衣服诊断板，及原尺寸样本。
- `parts/`：自动导出的 270 份独立 SVG 部件与 manifest。
- `samples/`：渲染器生成的组合样本，不是 Runtime 固定整图素材库。
- `review.json`：自动检查结果；不等同于美术认可。
