...
`resident-visual-review` 包含真实桌面UI、完整Catalog契约、腮红与Hair Coverage、96/64/48px、六框架自由创作保存测试。8A图板在 `avatar/phase8a/`；8B1的八张发型/衣装图板、八张桌面UI、固定配方旧新对照在 `avatar/phase8b/`，源码与SHA一起保存。移动端不再截图；CI PASS不是最终美术定稿。
