...
这一版是真实部件组合与对象保存：只有 **脸型、头发、衣服、表情** 四类。不是切换整张角色图，也不提供帽子、调色或五官滑杆。编辑器现在有三套原创 SVG 画风包：`linework-v1`（日常线绘）、`chibi-cute-v1`（Q版可爱）与 `simple-flat-v1`（极简简笔）。三套当前仍各有 4 脸、6 发、4 衣、6 表情，但这些选项已经改为 **Pack-owned Catalog**：以后 Q版可以独立扩 Hair / Outfit / Expression，不要求另外两套同步补画。画风仍不是第五类捏脸参数；人物年龄与性别继续来自目标对象。
