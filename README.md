...
`resident-visual-review` Artifact 的 `avatar/` 包含实际编辑窗口、已应用居民面板、六上下文诊断板与原尺寸样本。`parts/` 是 270 份自动展开的独立 SVG 部件及 manifest；`samples/` 是渲染器组合样本，不是 Runtime 固定整图素材库；`review.json` 和 `integration-review.json` 记录自动结果。
