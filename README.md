...
- `resident-snapshot.json`：由固定 CitySeed 生成的居民与家庭快照。
...
居民面板提供住处、工作地、家人入口；当前 LifeEvent 完整展示，旧 Stage 只作为同一事件的前情，Routine 以更低视觉级别显示。LifeEvent 的 `source` 可以把城市工程、家庭、营生和天气变化连接到具体居民。

详细规则见 `Documentation/居民面板与生活事件V2.md`。
