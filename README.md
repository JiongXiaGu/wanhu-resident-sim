...
这一版是真实部件组合与对象保存：只有 **脸型、头发、衣服、表情** 四类。不是切换整张角色图，也不提供帽子、调色或五官滑杆。编辑器现在有两套原创 SVG 画风包：`linework-v1`（日常线绘）与 `chibi-cute-v1`（Q版可爱）；两套都使用同一组 4 脸、6 发、4 衣、6 表情语义，并支持男女儿童/成人/老人上下文。画风是绘制方式，不是第五类捏脸参数。

编辑器左边选择玩家示例档案或当前城市任意居民，右边先选画风，再选择四类部件。切换画风保持同一组 face / hair / outfit / expression ID；Q版包会真正改变头身比例、五官画法、线条粗细和整体轮廓，不是给原画加滤镜。点击 **应用** 才写入这个对象；未保存修改可撤销，切对象或关闭会提示。给甲改头像不会改变乙。关闭工坊后，实际居民信息面板显示刚应用的头像，游戏日期和故事不会重置。
...
`resident-visual-review` Artifact 的 `avatar/` 包含实际编辑窗口、已应用居民面板、六上下文诊断板与原尺寸样本。原 `parts/` / `samples/` 检查日常线绘；`avatar/chibi-cute/` 单独包含Q版包的部件、组合样本、成人脸/头发/服装/表情图板与同配置跨画风对照。两套包各检查 3,456 个基础组合；组合数量只代表覆盖，不代表美术定稿。
