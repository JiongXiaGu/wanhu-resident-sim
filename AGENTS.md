...
先读 `Documentation/Phase 8 头像美术体系与创作工坊.md`。Phase 7 的美术没有被用户认可为最终画法；结构和兼容成果继续保留。用户在查看 8A 样板后已明确允许继续。当前执行 8B1：重画儿童与老年现有全部 Hair / Outfit，不扩大 ID 数量；成年其余素材留给 8B2。允许继续不等于每件美术已经最终定稿。
...
移动端不再截图或列为人工必查；基础窄屏溢出 smoke 可以保留。本轮全部新图板与真实桌面 UI 必须下载打开，包括旧新对照。工坊“已重画”表示累计重画候选，不表示用户已定稿。旧图通过 Git 历史临时生成，不在运行时复活旧资源。
