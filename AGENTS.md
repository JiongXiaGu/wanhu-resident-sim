...
不再制作“完整人物图画廊，然后以后再拆模块”的交付。当前 `linework-v1` 与 `soft-paint-v1` 都在 `Web/src/avatar/packs/` 下提供真实分层 SVG；画风切换必须保留四个语义选项，不能跨包复用整张角色图或用滤镜冒充新画风。画风仍未正式定稿，不把生成数量、SVG 复杂度或 CI PASS 当作审美结论。
