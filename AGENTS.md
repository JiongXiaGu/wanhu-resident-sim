...
不再制作“完整人物图画廊，然后以后再拆模块”的交付。当前 `linework-v1` 与 `chibi-cute-v1` 都在 `Web/src/avatar/packs/` 下提供真实分层 SVG；Q版包必须在头身比例、五官、轮廓线和发型体积上明显区别于线绘，不接受只换配色/阴影。画风切换仍保留四个语义选项，不能跨包复用整张角色图。画风仍未正式定稿，不把生成数量、SVG 复杂度或 CI PASS 当作审美结论。
