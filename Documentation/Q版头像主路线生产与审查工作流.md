...
- [x] Phase 7A：问题盘点与腮红安全区作者契约
- [x] Phase 7B：现有资产美术精修与 Actions / 人工截图审查
- [ ] Phase 7：用户最终美术验收（不是 Unity 迁移）

**当前主任务：Phase 7 候选美术交付与用户视觉验收。** 不自动进入 Unity 迁移或下一轮批量扩素材。

本次已完成问题盘点、腮红重画、男女 / 老年造型与衣领肩线精修。详见 [Phase 7 头像美术精修与验收](Phase%207%20头像美术精修与验收.md)。下方 Phase 1～5 的多 Pack / 过渡素材说明是历史阶段记录；当前运行时事实以本节、Phase 6 和 Phase 7 为准。
...
## 2026-09-20：Phase 7 美术精修候选与审查

本轮从 `9388d462993c27c18ca045ca0436d1b7550a98bd` 接手，沿用 Phase 6，不重做系统。

- 腮红从 FaceBase 移到唯一 Expression 作者组，缩小并内收；192 个 Frame × Face × Expression 组合通过实际脸轮廓与安全区检查，3 个故意越界负对照均被拒绝。没有运行时裁切或自动适配。
- 成年男女通过眉眼、下颌、发际和肩线区分；老年增加克制的年龄线，并重画专属低髻、盘髻、退后发际与贴耳鬓发。
- 既有 Outfit 重画连续交领、圆领、夹袄与围襟，保留 base / collar / overlay / detail；不再依赖对称尖领和中央领带色块。
- 六个上半头型参数、耳位、Coverage probes 不变；Hair 不读取 Face；单 Pack、四分类、10 个 compatibility-only ID 与保存流程不变。
- 候选 `86a51b3204de979a82ca05a55169dd0c31a0b926` 的 Build `35524723837`、Resident Visual Review `35524723840` 均成功。已下载 Artifact `10609239616`，实际打开五张指定 UI、三年龄综合图、统一色彩形体对照、腮红安全区和实际居民面板。
- 现有审查记录 11,136 个有效组合、220 个 Hair 覆盖点、176 项跨 Face 头部 geometry 检查通过。人工仅审查关键图，不宣称逐张人工看过全部组合。

人工结论：已查看样本没有明显腮红越界、露顶或断领；96 / 64px 的男女及年龄差异不只来自颜色，48px 主轮廓仍可读。48px 不承担细纹识别。男性较平的眼形、老年细纹强度与整体衣装审美仍属于候选，待用户确认；技术 PASS 不是最终美术定稿。

完整盘点、作者规则与截图路径见 [Phase 7 头像美术精修与验收](Phase%207%20头像美术精修与验收.md)。最终文档聚合提交及 main 必须再次跑同 SHA 的 Build / Visual Review，并下载抽查，不用本节候选 run 代替发布验证。
...
```text
Phase 7 — 现有 Q版资产美术精修
状态：7A / 7B 已完成候选；远端检查与关键截图人工审查完成，待用户最终美术验收
范围：保留 Phase 6 架构；不扩素材，不做 Unity 迁移
```
