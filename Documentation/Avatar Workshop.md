...
`linework-v1`（日常线绘）与 `chibi-cute-v1`（Q版可爱）都是原创、真实分层的 SVG 画风包。每包共享同一组 4 种面容语义、6 种发型、4 套衣服、6 种表情，并支持 female/male × child/adult/elder 六个作者上下文。画风选择只改变绘制方式，不新增第五类捏脸参数。人物年龄与性别取自目标对象，不由头像编辑改变。两份玩家示例档案分别是 21 岁女子与 32 岁男子，不冒充真实玩家资料。
...
`audit-avatar-art.mjs` 继续验证日常线绘包；`audit-avatar-chibi.mjs` 对 Q版包独立执行相同的共享部件、图层、SVG 合法性、眼睛/嘴型和全组合检查，并额外输出同配置跨画风对照。每个包每个 Frame 有颈部 1、FaceBase 4、前后发 12、衣服 4、脸型对应表情 24；这些是作者源规则的确定性展开，不代表把完整人物图切成了若干文件。
...
- 每个画风包分别执行六上下文 × 4 脸 × 6 发 × 4 衣 × 6 表情，即每包 3,456 个基础组合的 SVG、层序与身份不变量。\n- 同一成人配置在 `linework-v1` 与 `chibi-cute-v1` 间切换时，四个语义 ID 保持不变，同时两套最终 SVG 必须真正不同。
...
未实现连续参数捏脸、表情动画、真实账号系统、服务端同步、Unity 引擎导入或商业级年龄连续性。两套画风都提供儿童老人可编辑作者版本，但 Q版包的核心审美验收仍以成年女性/男性为主；年龄辨识、脸型差异、男女成熟度和 48/64px 表现仍须实际看截图。不能以组合数量、导出数量或 CI 绿色宣布美术已定稿。
