...
首包 `linework-v1`（日常线绘）是原创 SVG 部件：4 种面容、6 种发型、4 套衣服、6 种表情。支持 female/male × child/adult/elder 六个作者上下文。人物年龄与性别取自目标对象，不由头像编辑改变。两份玩家示例档案分别是 21 岁女子与 32 岁男子，不冒充真实玩家资料。
...
`audit-avatar-art.mjs` 从实际模块导出独立透明 SVG 部件、manifest 和组合样本：每个 Frame 有颈部 1、FaceBase 4、前后发 12、衣服 4、脸型对应表情 24，共 45 份；六个上下文共 270 份。它们是作者源规则的确定性展开，不代表画师手工画了 270 张互不相同的插画。
...
- 六上下文 × 4 脸 × 6 发 × 4 衣 × 6 表情，3,456 个基础组合的 SVG、层序与身份不变量。
...
未实现连续参数捏脸、表情动画、真实账号系统、服务端同步、Unity 引擎导入或商业级年龄连续性。儿童老人有可编辑作者版本，但年龄辨识和不同脸型的美术质量仍须实际验收。不能以组合数量、导出数量或 CI 绿色宣布美术已定稿。
