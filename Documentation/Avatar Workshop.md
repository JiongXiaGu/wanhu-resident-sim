...
8D1 新增常服/劳作/商铺/行旅各 4 Hair 和 4 Outfit ID，32 个新 ID 对应 44 份适用成年画稿。童老未扩量，不把成年画稿缩放给其他年龄。具体边界和审查见 [Phase 8D1 主题资产与文字检索](Phase%208D1%20主题资产与文字检索.md)。

100 个可选资产均有中文 `description` 和 `tags`；Catalog 是文字、适用性与兼容信息权威。短注在卡片中显示，完整说明随选中部件显示，卡片悬停也能查看。主题是美术/检索标签，不限制职业或跨主题搭配。

## 检索与草稿

部件栏搜索匹配当前分类、当前 Frame 和当前重画筛选内的 ID、名称、短注、完整描述、关键词。空格分词为 AND，NFKC 归一化，大小写不敏感。可搜 `8D1` 看本批，搜 `8D1 行旅` 看本批行旅，或输入 `斗笠`、`披肩`、完整 ID。
