...
Phase 7 的技术成果保留，但用户未认可其最终画法。当前进入 [Phase 8 头像美术体系与创作工坊](Phase%208%20头像美术体系与创作工坊.md)：8A 与 8B1 已交付，当前 8B2 重画剩余成年 12 Hair / 9 Outfit ID，并整批修正三款童老衣装。详见 [8B2 审查说明](Phase%208B2%20成年衣装与衣领审查.md)。

UI 中“已重画”表示8A、8B1 与 8B2 累计重画候选，不代表用户已最终验收；当前已有素材均进入累计清单；未来新增素材不自动标记。日常样板只载入草稿。随机本类仅修改当前分类，随机搭配只修改 Hair / Outfit。
...
model/store/render：Recipe、目标默认、保存和统一层序。Integration：App 内的目标与入口。AvatarEditor：草稿、目标切换、保存与部件浏览。studio.tsx：六框架上下文与累计样板标记。packs/chibi：Catalog 与静态画稿；adult-hair / adult-outfits 负责成年，sample-hair / sample-outfits 保留 8A 成年样板；child-hair / elder-hair / age-outfits 负责童老。hair / outfits 只分发。rework-batch 列出明确批次 ID，不代替 Catalog，不注册隐藏旧 Pack。
...
8B1 增加四框架全部素材点选、重画过滤、旧新同配方对照；8B2 接手后改为保留 8A 成年 11 份固定配方不变，另增加相对 8B1 全部 88 配方的变更范围检查。旧图通过Git历史临时导出，只在Actions Artifact中保留，不进入Web Pack。

未做：8C 年龄/性别入口和当前对象清晰度交互优化、独立帽子分类、连续参数捏脸、自动 Hair fit、服务端同步、Unity Runtime/正式存档迁移。
