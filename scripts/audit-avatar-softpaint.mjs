...
const out='review-screenshots/avatar/soft-paint';
await mkdir(join(out,'parts'),{recursive:true});
await mkdir(join(out,'samples'),{recursive:true});

export async function auditSoftPaint(page){
 const data=await page.evaluate(async()=>{
  ...
  const pack='soft-paint-v1',parts=[],samples=[],boards=[],wardrobe=new Map();
  let combinations=0,mouthChecks=0,eyeChecks=0;
  for(const frame of m.frames){
   ...
   for(const face of m.options.face){
    ...
    for(const expression of m.options.expression){
     ...
     const mouth=host.querySelector('[data-mouth]');require(mouth,'Soft expression lost mouth metadata');
     const box=mouth.getBBox();require(Math.abs(box.x+box.width/2-160)<.03,'Soft mouth shifted off center');mouthChecks++;
     const white=host.querySelector('[data-eye-white] path'),iris=host.querySelector('[data-eye-iris]');
     if(white&&iris){for(const item of iris.children){const b=item.getBBox();for(let x=b.x+.3;x<b.x+b.width;x+=.7)for(let y=b.y+.3;y<b.y+b.height;y+=.7){const pt=new DOMPoint(x,y);if(item.isPointInFill(pt))require(white.isPointInFill(pt),`Soft iris escaped eye: ${frame}/${face.id}/${expression.id}`);}}eyeChecks++;}
     for(const hair of m.options.hair)for(const outfit of m.options.outfit){
      ...
      require(ls.find(x=>x.id==='FaceBase').svg===faceSvg,'Soft wardrobe changed face');
      require(ls.find(x=>x.id==='Expression').svg===expressionSvg,'Soft wardrobe changed expression');
      const key=`${frame}/${hair.id}/${outfit.id}`,shared=JSON.stringify(ls.filter(x=>!['FaceBase','Expression'].includes(x.id)));
      if(wardrobe.has(key))require(wardrobe.get(key)===shared,'Soft face changed shared wardrobe');else wardrobe.set(key,shared);
      require(ls.map(x=>x.id).join(',')==='BackHair,Neck,Outfit,FaceBase,Expression,FrontHair','Soft layer order changed');
      require(!/<image\b|<script\b|<foreignObject\b|<mask\b|<clipPath\b/.test(svg),'Soft pack used forbidden complete-image or auto-fit constructs');
      require(!parser.parseFromString(svg,'image/svg+xml').querySelector('parsererror'),'Soft pack emitted invalid SVG');
      combinations++;
     }
    }
   ...
   require(faceGeometry.size===4,'Soft faces are not four independent outlines');
   if(frame.endsWith('adult')){
    boards.push({name:`faces-${frame}`,title:`柔光插画 · ${frame} · 四张脸`,columns:4,cells:faces});
    boards.push({name:`expressions-${frame}`,title:`柔光插画 · ${frame} · 六套表情`,columns:6,cells:expressions});
    boards.push({name:`hair-${frame}`,title:`柔光插画 · ${frame} · 共享六种头发`,columns:6,cells:hairs});
    boards.push({name:`outfits-${frame}`,title:`柔光插画 · ${frame} · 共享四套衣服`,columns:4,cells:outfits});
   }else boards.push({name:`age-${frame}`,title:`柔光插画 · ${frame} · 年龄上下文`,columns:4,cells:faces});
  }
  require(combinations===3456,'Soft pack must cover 3456 combinations');
  for(const frame of ['female.adult','male.adult']){
   const semantic={...m.defaultRecipe,face:'oval',hair:frame.startsWith('male')?'crop':'long',outfit:frame.startsWith('male')?'shirt':'knit',expression:'smile'};
   const line=r.renderAvatar(frame,{...semantic,pack:'linework-v1'}),soft=r.renderAvatar(frame,{...semantic,pack:'soft-paint-v1'});
   require(line!==soft,'Style packs rendered identical SVG');
   boards.unshift({name:`style-compare-${frame}`,title:`${frame} · 同一四项配置，不同画风包`,columns:2,cells:[{label:'日常线绘',svg:line,sizes:true},{label:'柔光插画',svg:soft,sizes:true}]});
  }
  host.remove();
  return {combinations,mouthChecks,eyeChecks,parts,samples,boards};
 });
 ...
 await writeFile(join(out,'parts','manifest.json'),JSON.stringify({pack:'soft-paint-v1',order:['BackHair','Neck','Outfit','FaceBase','Expression','FrontHair'],count:data.parts.length,parts:data.parts.map(({svg,...rest})=>rest)},null,2));
